...
function lock_edit(msg)
  ...
  if redis:get("lock:edit:"..msg.to.id) then
    return 'Edit Status Already Have : locked'
  else
    ...
    return 'Edit Status switched to : locked'
  end
end
...
function unlock_edit(msg)
  ...
  if not redis:get("lock:edit:"..msg.to.id) then
    return 'Edit Status Already Have : unloced'
  else
    redis:del("lock:edit:"..msg.to.id)
    return 'Edit Status switched to : unlocked'
  end
end
...
function run(msg,matches)
  ...
  if matches[1] == "lock" then
    ...
    return lock_edit(msg)
      ...
      return "You can't Manage Bot in Private"
    end

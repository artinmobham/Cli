...
local support_id = msg.from.id
 if matches[1]:lower() == 'id' and msg.to.type == "chat" or msg.to.type == "user" then
  ...
  if matches[1]:lower() == 'unban' or matches[1] =='حذف بن' then -- /unban
    if type(msg.reply_id)~="nil" and is_momod(msg) then
...
if matches[1]:lower() == 'kick' or matches[1] =='اخراج' then
      ...
      if string.match(targetuser, '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	banall_user(targetuser)
       		return 'کاربر ['..user_id..' ] کاملا مسدود شد | دیتابیس اپدیت شد'
     else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'banall',
  ...
  if matches[1]:lower() == 'unbanall' or matches[1] =='حذف گلوبال بن' then -- Global unban
    local user_id = matches[2]
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	return 'کاربر ['..user_id..' ] از مسدودیت کلی خارج شد'
    else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'unbanall',
